 ...
 *
 * <p>By contract promises to call the statements without transaction. Caller responsibility is to have or not to have
 * transactions around.
 *
...
public interface DbAccessDirect {
     ...
     *
     * <p>Use this when you expect multiple rows but only need the first one. Consider adding ORDER BY to ensure
     ...
     *
     * <p>Use this for large result sets where memory efficiency is important. Each row is mapped and passed to the
    ...
    /**
     * Identifier of this database connection, as configured on {@link DbConfig}. Useful for logs, metrics and
     * distinguishing between multiple databases in the same app.
     */
    String getId();
}
