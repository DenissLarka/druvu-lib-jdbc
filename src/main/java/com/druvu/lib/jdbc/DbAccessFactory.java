...
/** @author Deniss Larka at 11 Aug 2020 */
@Slf4j
public final class DbAccessFactory {
    ...
    /**
     * Variant for drivers that do not support JDBC transactions — primarily ClickHouse, which rejects both
     * {@code setAutoCommit(false)} (made by Spring's TX manager) and {@code setReadOnly(false)} (made by the Tomcat
     * pool on every borrow when its {@code defaultReadOnly} property is non-null).
     *
     * <p>This factory addresses both:
     *
     * <ul>
     *   <li>Returns {@link DbAccessDirect} (no TX manager wired) — the narrower interface also makes it a compile-time
     *       error to call {@code inTransaction} on a connection that cannot provide one.
     *   <li>Forces {@code defaultReadOnly=null} on the pool so Tomcat skips the {@code setReadOnly} call entirely. This
     *       overrides whatever the caller set, because the only realistic reason to use this factory is a driver/server
     *       (ClickHouse, analytical stores) that rejects the call.
     * </ul>
     *
     * <p>Use {@link #create(DbConfig)} for normal relational databases (PostgreSQL, Oracle, …) — they support both
     * transactions and {@code setReadOnly}.
     */
