...
/** @author Deniss Larka on 11 Jul 2022 */
public class TestDbAccess {
    ...
    public void testSelectFirstReturnsFirstRow() {
        ...
        Optional<Map<String, Object>> result =
                dbAccess.selectFirst(SimpleSql.fromString("SELECT * FROM TABLE1 ORDER BY ID_COL"));
    ...
    public void testScalarCountWithParams() {
        ...
        Integer count = dbAccess.selectOne(
                        SimpleSql.scalar("SELECT COUNT(*) FROM TABLE1 WHERE ID_COL > ?", Integer.class)
                                .with(0))
                .orElse(0);
    ...
    public void testScalarString() {
        String value = dbAccess.selectOne(
                        SimpleSql.scalar("SELECT FIRST_COL FROM TABLE1 WHERE ID_COL = ?", String.class)
                                .with(1))
                .orElse(null);
    ...
    public void testScalarLong() {
        Long id = dbAccess.selectOne(SimpleSql.scalar("SELECT ID_COL FROM TABLE1 WHERE FIRST_COL = ?", Long.class)
                        .with("value"))
                .orElse(null);
    ...
    public void testScalarReturnsEmptyWhenNoRows() {
        Optional<Integer> result =
                dbAccess.selectOne(SimpleSql.scalar("SELECT ID_COL FROM TABLE1 WHERE ID_COL = ?", Integer.class)
                        .with(999));
    ...
    public void testQueryWithLambdaMapper() {
        List<TestEntity> results = dbAccess.select(SimpleSql.query(
                "SELECT ID_COL, FIRST_COL FROM TABLE1",
    ...
    public void testQueryWithLambdaMapperAndParams() {
        ...
        List<TestEntity> results = dbAccess.select(SimpleSql.query(
                        "SELECT ID_COL, FIRST_COL FROM TABLE1 WHERE ID_COL > ? ORDER BY ID_COL",
                        (rs, rowNum) -> new TestEntity(rs.getInt("ID_COL"), rs.getString("FIRST_COL")))
                .with(1));
    ...
    public void testQueryWithSelectOne() {
        Optional<TestEntity> result = dbAccess.selectOne(SimpleSql.query(
                        "SELECT ID_COL, FIRST_COL FROM TABLE1 WHERE ID_COL = ?",
                        (rs, rowNum) -> new TestEntity(rs.getInt("ID_COL"), rs.getString("FIRST_COL")))
                .with(1));
    ...
    public void testNamedParametersSelect() {
        List<Map<String, Object>> results = dbAccess.select(
                SimpleSql.named("SELECT * FROM TABLE1 WHERE ID_COL = :id").with("id", 1));
    ...
    public void testNamedParametersMultipleParams() {
        ...
        List<Map<String, Object>> results = dbAccess.select(SimpleSql.named(
                        "SELECT * FROM TABLE1 WHERE ID_COL > :minId AND FIRST_COL LIKE :pattern ORDER BY ID_COL")
                .with("minId", 0)
                .with("pattern", "value%"));
    ...
    public void testNamedParametersWithMapper() {
        Optional<TestEntity> result =
                dbAccess.selectOne(SimpleSql.named("SELECT ID_COL, FIRST_COL FROM TABLE1 WHERE ID_COL = :id")
                        .with("id", 1)
    ...
    public void testNamedParametersWithMapParams() {
        List<Map<String, Object>> results = dbAccess.select(
                SimpleSql.named("SELECT * FROM TABLE1 WHERE ID_COL = :id").with(Map.of("id", 1)));
    ...
    public void testStreamProcessesAllRows() {
        ...
        dbAccess.stream(
                SimpleSql.query(
                        "SELECT ID_COL, FIRST_COL FROM TABLE1 ORDER BY ID_COL",
                        (rs, rowNum) -> new TestEntity(rs.getInt("ID_COL"), rs.getString("FIRST_COL"))),
                collected::add);
    ...
    public void testNamedParameterWithNullValue() {
        ...
        final Map<String, Object> row = dbAccess.selectOne(SimpleSql.fromString("SELECT * FROM TABLE1 WHERE ID_COL = ?")
                        .with(1))
                .orElseThrow();
    ...
    public void testTypedNamedParameterWithNullValue() {
        ...
        final List<Map<String, Object>> rows =
                dbAccess.select(SimpleSql.fromString("SELECT * FROM TABLE1 WHERE FIRST_COL IS NULL"));
    ...
    public void testPositionalNullParameter() {
        dbAccess.update(new SqlStatement<>(
                (rs, rowNum) -> "unused",
                "UPDATE TABLE1 SET FIRST_COL = ? WHERE ID_COL = ?",
                Arrays.asList((Object) null, 1)));
    ...
    public void testSelectFirstNullScalar() {
        final Optional<Integer> result =
                dbAccess.selectFirst(SimpleSql.scalar("SELECT MAX(ID_COL) FROM TABLE1 WHERE ID_COL > ?", Integer.class)
                        .with(999));
    ...
    private DbAccess createDb() {
        ...
        final DbConfig config = DbConfig.of(
                "testDb", "jdbc:h2:mem:mockChanges;MODE=PostgreSQL", "sa", "", "org.h2.Driver", "select 1 from dual");

        final DbAccess support = DbAccessFactory.create(config);

        support.update(SimpleSql.fromString("DROP ALL OBJECTS"));
        support.update(
                SimpleSql.fromString("CREATE TABLE TABLE1 (ID_COL INT NOT NULL PRIMARY KEY, FIRST_COL VARCHAR2(20))"));
