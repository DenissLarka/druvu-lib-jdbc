...
public class TestSimpleSqlBuilder {
    ...
    public void testWithMultipleParams() {
        SimpleSqlBuilder builder =
                SimpleSql.fromString("SELECT * FROM t WHERE a = ? AND b = ?").with(1, 2);
    ...
    public void testWithNullArgs() {
        SimpleSqlBuilder builder = SimpleSql.fromString("SELECT 1").with((Object[]) null);
    ...
    public void testWithNullValue() {
        SimpleSqlBuilder builder =
                SimpleSql.fromString("SELECT * FROM t WHERE a = ?").with((Object) null);
    ...
    public void testBuilderIsStatement() {
        ...
        SqlStatement<Map<String, Object>> statement =
                SimpleSql.fromString("SELECT * FROM users").with(1);
    ...
    public void testFromResource() {
        SimpleSqlBuilder builder = SimpleSql.fromResource("sql/test4.sql").with("value1", "value2", 3);

        Assert.assertEquals(
                builder.getQuery(), "select col1, col2, col3 from table1 where col1=? and col2=? and col3=?");
        Assert.assertEquals(builder.getParameters().length, 3);
    }
