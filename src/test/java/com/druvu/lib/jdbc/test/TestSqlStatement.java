...
/** @author Deniss Larka at 31 Jul 2020 */
public class TestSqlStatement {
    ...
    public void test1() {
        SqlStatement<String> sqlStatement = new SqlStatement<>() {
            ...
            public Object[] getParameters() {
                return ArrayUtils.flat("one", "two", Arrays.asList("three", Arrays.asList("four", "five")))
                        .toArray();
            }
        ...
        Assert.assertEquals(
                ...
                "select col1, col2, col3 from table1 where col1=? and col2=? and col3 in (?,?,?)");
    }
    ...
    public void testDebugSpecialCharacters() {
        final String sql =
                SqlDebug.debug("select * from t where a=? and b=?", new Object[] {"price $5", "back\\slash"});
    ...
    public void testConstructorAllowsNullArguments() {
        final SqlStatement<String> statement = new SqlStatement<>(
                (rs, rowNum) -> "x", "update table1 set col1=? where col2=?", Arrays.asList((Object) null, 5));
